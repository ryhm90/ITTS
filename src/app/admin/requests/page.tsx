...
export default function AdminDashboard() {
  ...
  const loadRequests = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/requests/all?page=${page}&pageSize=${pageSize}`, {
        credentials: 'include',
      });
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/requests?deviceId=${detail.deviceId}&serviceName=${detail.service}&depName=${detail.departmentName}&divName=${detail.divisionName}&page=${drPage + 1}&pageSize=${drRowsPerPage}`, { credentials: 'include' })
      .then(async (r) => {
        if (!r.ok) return [];
...
React.useEffect(() => {
  ...
  return (
    <PageContainer>
      {/* رأس الصفحة */}
      <HeaderRow>
        <PageTitle>كل الطلبات</PageTitle>
      </HeaderRow>
...
{r.deviceType}#{r.deviceNo}
                    {r.deviceDesc && ` (${r.deviceDesc})`}                  </TableBodyCell>
