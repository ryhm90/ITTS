// src/app/api/unit/requests/route.ts
...
export async function GET(req: NextRequest) {
  ...
  // 2) استخراج معرف الوحدة من الـ JWT
  ...
  const ps = db.request()
    .input('secId',  sql.Int, unitId)    // <— تصحيح: يجب להיות sectionId כאן
  ...
  let query = `
    ...
    FROM dbo.Requests r
    INNER JOIN dbo.CurrentHandlerRoleUnit chd
